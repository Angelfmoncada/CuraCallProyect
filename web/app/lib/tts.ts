...
export async function speakWithCoqui(
  ...
  try {
    ...
    if (!process.env.NEXT_PUBLIC_API_BASE_URL && base.includes('localhost')) {
      // Demo mode - return null or throw error to disable TTS
      throw new Error('TTS not available in demo mode');
    }
    ...
    if (CURRENT_AUDIO) {
      const audio = CURRENT_AUDIO as HTMLAudioElement;
    ...
    audio.onended = () => {
     ...
     // In demo mode or when backend is not available, disable TTS
     console.warn('TTS not available:', error instanceof Error ? error.message : 'Unknown error');
     throw new Error('TTS service not available in demo mode');
   }
}
